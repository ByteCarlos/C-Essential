...
void criarConta(){
    ...
    strcat(data_cadastro, dia);// 23
    strcat(data_cadastro, "/");// 23/
    strcat(data_cadastro, mes);// 23/11
    strcat(data_cadastro, "/");// 23/11/
    strcat(data_cadastro, ano);// 23/11/2021
...
void transferir(Conta conta_origem, Conta conta_destino, float valor){
    if(contador_contas > 0){
        ...
        printf("Não existe contas cadastradas ainda.\n");
    }
}
